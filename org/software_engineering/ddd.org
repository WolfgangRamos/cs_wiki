...
- Hide orchestration of business rules in entities, value objects and
  specifications.
...
- Instead of implementing a business rule as a method on a domain
  object, business rules can be modelled as _specification_ domain
  object, that encapsulates that business rule.
- _Specifications_ are first class domain objects.
- Example: Instead of having a =Discount(Price price)= method on a
  =Customer=, create a =Discount= class that provides an =Apply(Price
  price)= method.
